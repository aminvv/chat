...
function VerifyRefreshToken(token) {
  return new Promise((resolve, reject) => {
    console.log("ورودی توکن:", token);
    JWT.verify(token, REFRESH_TOKEN_SECRET_KEY, (err, payload) => {
      if (err) {
        console.log("خطا در verify:", err.message);
        return reject(createError.Unauthorized("ورود مجدد به حسابی کاربری انجام نشد"));
      }
      console.log("payload:", payload);
      const { mobile } = payload || {};
      if (!mobile) return reject(createError.Unauthorized("موبایل در توکن یافت نشد"));
      resolve(mobile);
    });
  });
}
